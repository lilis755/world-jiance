...
// Focus on high-signal situational awareness up top; keep news lower and optionally hidden.
const FULL_PANELS: Record<string, PanelConfig> = {
  map: { name: '全球地图', enabled: true, priority: 1 },
  'scenario-brief': { name: '场景简报', enabled: true, priority: 1 },
  monitors: { name: '我的监控', enabled: true, priority: 1 },
  'security-advisories': { name: '安全警告', enabled: true, priority: 1 },
  cii: { name: '国家不稳定性', enabled: true, priority: 1, ...(_desktop && { premium: 'enhanced' as const }) },
  'strategic-risk': { name: '战略风险概览', enabled: true, priority: 1, ...(_desktop && { premium: 'enhanced' as const }) },
  'strategic-posture': { name: 'AI战略态势', enabled: true, priority: 1 },
  insights: { name: 'AI洞察', enabled: true, priority: 1 },
  cascade: { name: '基础设施级联风险', enabled: true, priority: 1 },
  intel: { name: '情报动态', enabled: true, priority: 1 },
  'gdelt-intel': { name: '实时情报', enabled: true, priority: 1, ...(_desktop && { premium: 'enhanced' as const }) },
  'service-status': { name: '服务状态', enabled: true, priority: 1 },
  'live-news': { name: '时事新闻大屏', enabled: true, priority: 1 },
  'live-webcams': { name: '实时摄像头', enabled: true, priority: 1 },
  'ucdp-events': { name: '冲突事件库', enabled: true, priority: 1 },
  displacement: { name: '人口流离', enabled: true, priority: 1 },
  climate: { name: '气候异常', enabled: true, priority: 1 },
  'population-exposure': { name: '人口暴露', enabled: true, priority: 1 },
  'satellite-fires': { name: '卫星火点', enabled: true, priority: 1 },
  'satellite-imagery': { name: '卫星影像', enabled: true, priority: 1 },
  'airline-intel': { name: 'Airline Intelligence', enabled: true, priority: 1 },
  'oref-sirens': { name: 'Israel Sirens', enabled: true, priority: 1, ...(_desktop && { premium: 'locked' as const }) },
  'telegram-intel': { name: '电报情报流', enabled: true, priority: 1, ...(_desktop && { premium: 'locked' as const }) },
  'macro-signals': { name: 'Market Radar', enabled: true, priority: 2 },
  polymarket: { name: '预测市场', enabled: true, priority: 2 },
  commodities: { name: '大宗商品', enabled: true, priority: 2 },
  markets: { name: '市场概览', enabled: true, priority: 2 },
  economic: { name: '经济指标', enabled: true, priority: 2 },
  'trade-policy': { name: '贸易政策', enabled: true, priority: 2 },
  'supply-chain': { name: '供应链风险', enabled: true, priority: 2, ...(_desktop && { premium: 'enhanced' as const }) },
  ...
  // News-heavy panels: hidden by default, available via panel selector
  politics: { name: 'World News', enabled: false, priority: 2 },
  us: { name: 'United States', enabled: false, priority: 2 },
  europe: { name: 'Europe', enabled: false, priority: 2 },
...
const TECH_PANELS: Record<string, PanelConfig> = {
  ...
  'service-status': { name: 'Service Status', enabled: true, priority: 1 },
